...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { type, data, id, metadata, statsType } = body;

    // Support both new format (type + data) and legacy format
    const finalStatsType = type || statsType;

    if (!id && !finalStatsType) {
      return NextResponse.json(
    ...
    if (id) {
      // Update by specific stats record ID (legacy)
      ...
      const existingStats = await prisma.stats.findFirst({
        where: { type: finalStatsType },
        orderBy: { updatedAt: 'desc' }
      });

      if (!existingStats) {
        return NextResponse.json(
          { message: `No stats found with type: ${finalStatsType}` },
          { status: 404 }
        );
      }
      ...
      if (type && data) {
        ...
        // Legacy format: replace entire metadata array
        if (!Array.isArray(metadata)) {
          return NextResponse.json(
            { message: 'metadata must be an array of objects' },
            { status: 400 }
